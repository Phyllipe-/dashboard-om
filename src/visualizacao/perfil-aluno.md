...
function _renderizarReplay() {
  ...
  if (mapaFinalizado) {
    for (const [si, oc] of objCenters.entries()) {
      // Objetivo pelo índice canônico (objCenters já está ordenado por distância
      // do início, igual ao sortInterByStart dos demais mapas) — garante 1:1.
      const obj = objetivos[si] ?? null;
